...
class SGTree{
    ...
    void update(int low, int high, vector<int>&arr, int ind, int val , int k){
        if(low==high){
            seg[ind] = val;
            return;
        }
        int mid = (low+high)/2;
        if(mid>=k)update(low,mid,arr,2*ind+1,val,k);
        else update(mid+1,high,arr,2*ind+2,val,k);
        seg[ind] = seg[2*ind+1]+seg[2*ind+2];
    }
};
